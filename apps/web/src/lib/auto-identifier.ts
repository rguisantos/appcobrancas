...
export async function generateNextIdentifier(
  ...
  // Find the highest existing identifier with this prefix
  const records = await (db[modelName] as any).findMany({
    where: {
  ...
  if (records.length === 0) {
    // No existing records with this prefix, start at 1
    return `${prefix}${String(1).padStart(padLength, '0')}`
  }

  // Extract the numeric part from the last identifier
  const lastIdentifier = records[0][fieldName] as string
  const numericPart = lastIdentifier.replace(prefix, '')
  const lastNumber = parseInt(numericPart, 10)

  if (isNaN(lastNumber)) {
    // If we can't parse it, try counting all records with this prefix
    const count = await (db[modelName] as any).count({
      where: {
        [fieldName]: { startsWith: prefix },
        deletedAt: null,
      },
