...
export function ClienteFormView() {
  ...
  // Load existing contatos on mount
  useEffect(() => {
    if (Array.isArray(formData.contatos) && formData.contatos.length > 0) {
      setContatosList(formData.contatos as Array<{nome: string; telefone: string; funcao?: string}>)
    }
  }, [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...formData,
        // Use contatosList (source of truth) instead of formData.contatos which may be stale
        contatos: contatosList.filter(c => c.nome || c.telefone),
